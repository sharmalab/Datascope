...
var _containsMarker = function(f, d) {
  var fSouthWest = f._southWest,
    fNorthEast = f._northEast;
  var dLatLng = d.split(",");
  var dLat = dLatLng[0],
    dLng = dLatLng[1];
...
var _filterFunction = function(filter, dataSourceName) {
  ...
  Object.keys(groups).forEach(function(key) {
    results[key] = {
      values: groups[key].all(),
      top: groups[key].top(1)[0].value
    };
  });
  var interactiveFiltersConfig = interactiveFilters.getInteractiveFiltersConfig();
  var filteredData = dimensions[interactiveFiltersConfig[0].attributeName].top(
    Infinity
  );
...
var _tableNext = function(req, res) {
  ...
  var TABLE_DATA = dimensions[interactiveFiltersConfig[0].attributeName]
    .top(start)
    .slice(start - 10);
...
var _getStatistics = function(req, res) {
  ...
  var TABLE_DATA = dimensions[interactiveFiltersConfig[0].attributeName].top(
    Infinity
  );
      ...
      if (statistics.constructor === String) {
        if (statistics === "default") {
          statistics = [
            "correlation",
            "rankCorrelation",
            /*"distanceCorrelation",*/ "dotProduct",
            "euclidianDistance",
            "covariance",
            "cohensd"
          ];
      ...
      if (statistics.constructor === Array) {
        statistics.forEach(function(stat) {
          if (stat.startsWith("custom")) {
            ...
            statisticsToReturn.rankCorrelation = dl.cor.rank(
              TABLE_DATA,
              attr1,
              attr2
            );
            ...
            statisticsToReturn.euclidianDistance = dl.dist(
              TABLE_DATA,
              attr1,
              attr2
            );
            ...
            statisticsToReturn.covariance = dl.covariance(
              TABLE_DATA,
              attr1,
              attr2
            );
