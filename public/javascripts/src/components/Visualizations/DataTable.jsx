...
var DataTable = React.createClass({
  ...
  componentDidMount: function() {
    ...
    for (var i in tableAttributes) {
      ...
      columns[count].title =
        self.props.config.attributes[i].label ||
        self.props.config.attributes[i].attributeName;
...
fetch("./configs/tableLinks.json").then(x=>x.json()).then(x=>{
  var _tableLinks = {}
  x.forEach(x=>{
    let p = x.position
    let pp = parseInt(p)+1
    console.log("td:nth-child("+pp+")")
    $("td:nth-child("+pp+")").addClass("tableLink");
    _tableLinks[p]=x
  })
  return _tableLinks;
}).then(x=>{
  $(".datatablevis")
    ...
    .delegate("td", "click", function(e) {
      ...
      if (x.hasOwnProperty(i)) {
        let prefix = x[i].prefix || ""
        let suffix = x[i].suffix || ""
        if (txt) {
          var url = prefix + txt + suffix;
  ...
  render: function() {
    ...
    var id = this.state.id;
    var tableId = "vis" + id;
    return <table id={tableId} className="display datatablevis" />;
  }
});
